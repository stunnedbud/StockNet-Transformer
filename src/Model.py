...
# Transformer layers
from multi_head_attention import MultiHeadAttention, LayerNormalization
...
class TransformerDecoderLayer(Layer):
    ...
    def __init__(self, H, d_model, dk, dv, dff, input_shp=(), dropout_rate=0.1, layernorm_eps=1e-6):
        ...
        self.mha1 = MultiHeadAttention(num_heads=H, key_dim=d_model)
        self.mha2 = MultiHeadAttention(num_heads=H, key_dim=d_model)
        # Feed forward network layer
        self.ffn = Sequential(
                            [Dense(dff, activation="relu", input_shape=input_shp),
                             Dense(d_model)])
    ...
    def build(self, input_shape):
        self.mha1._build_from_signature(input_shape, input_shape, key=None)
        self.mha2._build_from_signature(input_shape, input_shape, key=None)

    def call(self, x, encoder_output=None, training=False, look_ahead_mask=None, padding_mask=None):
        """
        ...
        A1 = self.mha1(x,attention_mask=look_ahead_mask) # Self attention (batch_size, Tq, d_model)
        A1 = self.dropout_mha1(A1, training=training) #Apply Dropout during training

        # 2nd Masked MultiHead attention
        A2 = self.mha2(x,value=encoder_output,key=encoder_output,attention_mask=padding_mask) # Self attention (batch_size, Tq, d_model)
        A2 = self.dropout_mha2(A2, training=training) #Apply Dropout during training

        # Pointwise ffn
        ffn_output = self.ffn(A2) # (batch_size, Tq, d_model)
...
class TransformerEncoderLayer(Layer):
    def __init__(self, total_heads, total_dense_units, embed_dim, input_shp=()):
        super(TransformerEncoderLayer, self).__init__()
        # Multihead attention layer
        self.multihead = MultiHeadAttention(num_heads=total_heads, key_dim=embed_dim) #, attention_axes=(1, 2))
        # Feed forward network layer
        self.nnw = Sequential(
                            [Dense(total_dense_units, activation="relu", input_shape=input_shp),
                             Dense(embed_dim)])

    def build(self, input_shape):
        self.multihead._build_from_signature(input_shape, input_shape, key=None)

    def call(self, inputs):
        attn_output = self.multihead(inputs)
        nnw_output = self.nnw(attn_output)
        return nnw_output
...
class Model:
    ...
    # En desuso en la version mas reciente, se reemplazo este modulo por un transformer
    ...
    # Cambio a la arquitectura realizado para el proyecto de Redes Profundas
    # Reemplazamos el modulo auxiliar temporal por un transformer
    def _build_transformer_temporal_att(self):
        """
            g: batch_size * max_n_days * g_size
            g_T: batch_size * g_size
        """
        num_heads = 8
        total_dense_units = 50
        embed_dim = 16
        with tf.name_scope('tda'):
            with tf.variable_scope('tda'):
                self.encoder_layer = TransformerEncoderLayer(num_heads, total_dense_units, embed_dim, input_shp=self.g.shape)  #self.g_size)

                H, d_model, dk, dv, dff = 8, 512, 16, 16, 50,
                self.decoder_layer = TransformerDecoderLayer(num_heads, embed_dim, dk, dv, dff, input_shp=self.g.shape)
                """
                ...
                # Conexion de las capas del transformer
                #keras_input_g = K.constant(v_i)
                #keras_input_g = K.constant(self.g  )  #tf.stack(self.g)
                #keras_input_g = Input(tensor=self.g) #shape=self.g_size)
                #pool = GlobalAveragePooling1D()(enc)
                #d = Dense(total_dense_units, activation="relu")(pool)
                #keras_g = tf.map_fn(K.constant, self.g) # necesitamos el map para ejecutar en el transformer de keras 2 los batches de tf, porque la longitud es desconocida antes de runtime
                enc = tf.map_fn(self.encoder_layer, self.g)
                dec = self.decoder_layer(self.g, encoder_output=enc)
                pool = GlobalAveragePooling1D()(dec)#(enc)
                aux_score = self._linear(pool, self.max_n_days, activation="softmax")  #Dense(self.max_n_days, activation="softmax")(d)  #tf.multiply(v_i, v_d, name='v_stared')

                # Operaciones de stocknet
                ninf = tf.fill(tf.shape(aux_score), np.NINF)
                masked_aux_score = tf.where(self.mask_aux_trading_days, aux_score, ninf)
                v_stared = aux_score #tf.nn.softmax(masked_aux_score)
                ...
                #att_c = tf.reduce_sum(tf.matmul(context, v_stared), axis=-1)  # batch_size * g_size / y_size
                with tf.variable_scope('y_T'):
                    self.y_T = self._linear(aux_score, self.y_size, 'softmax')

# Opcion 2 de modificacion a la arquitectura
# Reemplazar el modulo auxiliar temporal por una capa de atencion multicabeza
def _build_multihead_temporal_att(self):
    """
        g: batch_size * max_n_days * g_size
        g_T: batch_size * g_size
    """
    ...
    def assemble_graph(self):
        ...
        with tf.device('/device:GPU:0'):
            ...
            self._build_mie()
            self._build_vmd()
            self._build_transformer_temporal_att() # ._build_multihead_temporal_att() # Esta linea selecciona qué modulo usar de atención temporal
            self._build_ata()
